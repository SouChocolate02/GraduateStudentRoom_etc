...
\begin{tcolorbox} %%R8_2nd_1
	$\fbox{1}$
\begin{enumerate}
    \item $V, W$を複素線形空間とし、$V, W$の零ベクトルをそれぞれ$\bm{0}_V, \bm{0}_W$とする。$V$から$W$への線形写像$T\colon V\to W$に対して、$T$の像を$\img{(T)}$とし、$T$の核を$\ker{(T)}$とする。$\img{(T)}$および$\ker{(T)}$の定義をそれぞれ述べなさい。
...
\begin{enumerate_alpha}
    ...
    \item $\img{(S)}$と$\img{(S)}$の和空間を$W= \img{(S)}+ \ker{(T)}$とおく。$W$の次元を求めなさい。またその根拠も述べなさい。
