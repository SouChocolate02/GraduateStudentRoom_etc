...
\begin{enumerate}
    \item 
\begin{equation}
    \int_{E_3} (f+ g)d\mu= \int_{E_3} fd\mu+ \int_{E_3} gd\mu
\end{equation}
    \item $A\subset E, \mu(A)= 0$ならば，$f$は$A$上可積分で$\int_A fd\mu= 0$である．
    \item $f$が$E$上可積分ならば$\mu(E(f= -\infty))= 0$
\end{enumerate}
...
\begin{tcolorbox}$\fbox{17}$
    $I= (a, b)$とする．関数$f= f(x, t),\ \ (t\in I)$は$x$の関数として$E$上可測，$\phi$は$E$上可積分で，すべての$t\in I$に対し，$|f(x, t)|\le \phi(x)\ (\mathrm{a.e.} x\in E)$をみたすとする．このときa.e.$x\in E$に対し，極限$\lim_{t\to a+ 0} f(x, t)$が存在するならば
\begin{equation}
    \lim_{t\to a+ 0} \int_E f(x, t)d\mu(x)= \int_E\left(\lim_{t\to a+ 0} f(x, t)\right)d\mu(x)
\end{equation}
    が成り立つことを示せ．
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tcolorbox}$\fbox{20}$
    $I= [0, 1], I\cap \Q= \{r_n\}_{n= 1}^\infty$とし，$I$上の関数$\phi_n$を$\phi_n(x)= \begin{cases}|x- r_n|^{-\frac{1}{2}}\ \ (x\in I\setminus\{r_n\})\\ + \infty\ \ (x= r_n)\end{cases}$で定める．また$f(x)= \sum_{n= 1}^\infty \frac{1}{n^2}\phi_n(x)$とおく．この問題では一次元Lebesgue測度に関する$I$におけるLebesgue積分を$\int_0^1\cdots dx$で表す．
\begin{enumerate}
    \item $\phi_n, f$が可測関数であることを示しなさい．
    \item $\int_0^1 \phi_n(x)dx$を計算し，$r_n$を含む形式で表しなさい．また$\int_0^1 \phi_n(x)dx\le 2\sqrt{2}$であることを示しなさい．
    \item $\int_0^1 f(x)dx= \sum_{n= 1}^\infty \frac{1}{n^2}\int_0^1 \phi_n(x)dx$が成り立つ理由を述べなさい．
    \item $x\in I\cap \Q$に対し，$f(x)= +\infty$であることを示しなさい．
    \item $f(x)< +\infty \ \mathrm{a.e.} x\in [0, 1]$が成り立つことを示しなさい．
\end{enumerate}
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tcolorbox}$\fbox{21}$
    $E$を$\R^N$の可測集合，$f\colon E\to \R$を$e^{|f|}$が$E$上可積分である可測関数とする．このとき以下を示しなさい．
\begin{enumerate}
    \item 0以上の任意の整数$n$に対して，$f^n$は$E$上可積分である．
    \item $e^f$は$E$上可積分である．
    \item $\int_E e^f d\mu= \sum_{n= 0}^\infty \frac{1}{n!}\int_E f^n d\mu$が成り立つ．
\end{enumerate}
...
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tcolorbox}$\fbox{24}$
    $\norm{・}_\infty$がノルムであることを示せ．
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tcolorbox}$\fbox{25}$
    非負値の$f\in L^p$に対して定理6.8の$f_\epsilon$が存在することを用いて，一般の$p$乗可積分関数$f\colon \R^N\to [-\infty, +\infty]$ならびに$f\colon \R^N\to \C$に対して定理6.8の$f_\epsilon$が存在することを示しなさい．
\end{tcolorbox}
\end{document}
